...
class ProventosAcaoFundamentusWsToBronze:

    def __init__(self, papel, url="https://www.fundamentus.com.br/proventos.php?"):
        self.papel = papel
    ...
    def get_fii_data_list(self):
        ...
        if tabela:
            ...
            for i, linha in enumerate(linhas[1:]):
                ...
                # print(i, colunas[i].get_text())

                # Extrair informações específicas das colunas desejadas
                tipo = colunas[2].get_text()
                data_pgto = colunas[3].get_text()
                valor = colunas[1].get_text()
    ...
    def run(self):       
        IngestionController("fundamentus/proventos/acao").write_bronze(self.get_dataframe())

# COMMAND ----------

tickers_gold = IngestionController("fundamentus/acao").read_gold("ufrj_tcc_2023").select("papel").distinct()
tickers_list = Utils.collect_col_values(tickers_gold, "papel")

df_list = []
for ticker in tickers_list:
    df = ProventosAcaoFundamentusWsToBronze(ticker).get_dataframe()
    df_list.append(df)

df = Utils.union_all_df(df_list)
IngestionController("fundamentus/proventos/acao").write_bronze(df)
